...
class TestCiscoApicL3Driver(testlib_api.SqlTestCase,
    ...
    def setUp(self):
        ...
        self.plugin._apic_driver._aci_mech_driver = mock.Mock()
    ...
    def test_create_floatingip_postcommit(self):
        ...
        fip_context = api.FipContext(fip, original=fip)
        apic_driver = self.plugin._apic_driver
        apic_driver.create_floatingip_postcommit(
            self.context, fip_context)
    ...
    def test_update_floatingip_precommit(self):
        fip_context = api.FipContext({'floatingip': self.floatingip})
        apic_driver = self.plugin._apic_driver
        apic_driver.update_floatingip_precommit(
            self.context, FLOATINGIP, fip_context)
        self.assertEqual(PORT, fip_context.port_id_list[0])

    def test_update_floatingip_postcommit(self):
        fip = {'floatingip': self.floatingip,
               'id': FLOATINGIP, 'port_id': PORT}
        fip_context = api.FipContext(fip, original=fip)
        fip_context.port_id_list = [self.port['id']]
        apic_driver = self.plugin._apic_driver
        apic_driver.update_floatingip_postcommit(
            self.context, FLOATINGIP, fip_context)
        self.assertEqual(self.port['id'], fip_context.port_id_list[0])
        self.assertEqual(PORT, fip_context.port_id_list[1])
    ...
    def test_delete_floatingip_precommit(self):
        fip_context = api.FipContext(None)
        apic_driver = self.plugin._apic_driver
        apic_driver.delete_floatingip_precommit(
            self.context, FLOATINGIP, fip_context)
        self.assertEqual(PORT, fip_context.port_id_list[0])

    def test_delete_floatingip_postcommit(self):
        fip_context = api.FipContext(None)
        fip_context.port_id_list = [PORT]
        apic_driver = self.plugin._apic_driver
        apic_driver.delete_floatingip_postcommit(
            self.context, FLOATINGIP, fip_context)
        self.ml2_driver.notify_port_update_for_fip.assert_called_once_with(
            PORT)
