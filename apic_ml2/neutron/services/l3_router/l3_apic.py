...
class ApicL3ServicePlugin(common_db_mixin.CommonDbMixin,
    ...
    def create_floatingip(self, context, floatingip):
        res = super(ApicL3ServicePlugin, self).create_floatingip(
            context, floatingip)
        fip_context = api.FipContext(res, original=floatingip)
        self._apic_driver.create_floatingip_postcommit(context, fip_context)
        return res

    def update_floatingip(self, context, id, floatingip):
        fip_context = api.FipContext(None, original=floatingip)
        ...
        fip_context.current = res
        self._apic_driver.update_floatingip_postcommit(
            context, id, fip_context)
        return res
    ...
    def delete_floatingip(self, context, id):
        ...
        self._apic_driver.delete_floatingip_postcommit(
            context, id, fip_context)
        return res
